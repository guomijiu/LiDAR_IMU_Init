...
void Preprocess::process_cut_frame_pcl2(const sensor_msgs::msg::PointCloud2::UniquePtr &msg, deque<PointCloudXYZI::Ptr> &pcl_out,
                                        deque<double> &time_lidar, const int &required_frame_num, int &scan_count)
{
  ...
  else if (lidar_type == HESAI)
  {
    ...
    float yaw_last[MAX_LINE_NUM] = {0.0};  // yaw of last scan point
    float time_last[MAX_LINE_NUM] = {0.0}; // last offset time
    double timeEnd = pl_orig.points[plsize - 1].timestamp;
    double timeStat = pl_orig.points[0].timestamp;
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.z = pl_orig.points[i].z;
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature = (pl_orig.points[i].timestamp - timeStat) * 1000.0; // ms
...
void Preprocess::default_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  pcl::PointCloud<hesai_ros::Point> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  auto ptStart = pl_orig.begin();
  double timeStart = ptStart->timestamp;
  ...
  for (int i = 0; i < pl_orig.points.size(); i++)
  {
    ...
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    added_pt.curvature = (-(timeStart - pl_orig.points[i].timestamp)) * 1000.0; // ms
